...
@section('content')
    ...
    <style>
        ...
        .bot-content-area ul li {
            margin-bottom: 0.4rem;
            padding-left: 0.25rem;
        }

        .bot-content-area ul li::marker {
            color: #059669;
        }
        ...
        .bot-content-area ol li {
            margin-bottom: 0.4rem;
            padding-left: 0.25rem;
        }

        .bot-content-area ol li::marker {
            color: #059669;
            font-weight: 700;
        }
        ...
        .dark .bot-content-area th,
        .dark .bot-content-area td {
            border-color: #334155;
        }
        ...
        .bot-content-area tr:nth-child(even) {
            background-color: #f1f5f9;
        }

        .dark .bot-content-area tr:nth-child(even) {
    ...
    <div x-data="chatbotComponent"
        class="h-[calc(100vh-120px)] max-w-6xl mx-auto flex bg-white dark:bg-slate-900 rounded-[2.5rem] border border-slate-200 dark:border-slate-800 shadow-xl overflow-hidden relative">
        ...
        {{-- ====================================================== --}}
        {{-- SIDEBAR KIRI: HISTORY DARI DATABASE                    --}}
        {{-- ====================================================== --}}
        <div :class="sidebarOpen ? 'translate-x-0' : '-translate-x-full'"
            class="absolute md:static inset-y-0 left-0 z-50 w-72 bg-slate-50 dark:bg-slate-950/50 border-r border-slate-200 dark:border-slate-800 flex flex-col transition-transform duration-300 md:translate-x-0">

            {{-- Tombol New Chat --}}
            <div class="p-4">
                <button @click="startNewChat()"
                    class="w-full flex items-center gap-3 px-4 py-3 bg-white dark:bg-slate-800 hover:bg-slate-100 dark:hover:bg-slate-700 border border-slate-200 dark:border-slate-700 rounded-xl text-sm font-bold text-slate-700 dark:text-slate-200 transition-colors shadow-sm">
            ...
            <div class="flex-1 overflow-y-auto sidebar-scroll px-3 pb-4">
                ...
                <template x-if="historyLoading">
                    <div class="flex items-center justify-center py-10 gap-2 text-slate-400 text-sm">
                ...
                {{-- State: History kosong --}}
                <template
                    x-if="!historyLoading && chatHistories.today.length === 0 && chatHistories.pastWeek.length === 0 && chatHistories.older.length === 0">
                ...
                <div class="space-y-5" x-show="!historyLoading">
                    {{-- Hari Ini --}}
                    <template x-if="chatHistories.today.length > 0">
                        <div>
                                ...
                                <template x-for="history in chatHistories.today" :key="history.id">
                                    <li>
                                        <button @click="loadHistory(history.id)"
                                            :class="currentSessionId === history.id ?
                                            ...
                                            <svg class="w-4 h-4 flex-shrink-0 opacity-60" fill="none"
                    ...
                    {{-- 7 Hari Terakhir --}}
                    <template x-if="chatHistories.pastWeek.length > 0">
                        <div>
                            <p class="px-3 text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">7 Hari Terakhir
                            </p>
                            <ul class="space-y-1">
                    ...
                    {{-- Lebih Lama --}}
                    <template x-if="chatHistories.older.length > 0">
                        <div>
                            <p class="px-3 text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">Lebih Lama</p>
                            <ul class="space-y-1">
                                <template x-for="history in chatHistories.older" :key="history.id">
                                    <li>
                                        <button @click="loadHistory(history.id)"
                                            :class="currentSessionId === history.id ?
                                                'bg-green-50 dark:bg-green-900/20 text-green-700 dark:text-green-400 font-semibold' :
        ...
        <div class="flex-1 flex flex-col h-full bg-white dark:bg-slate-900 relative">
            ...
            <div class="px-6 py-4 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between">
                <div class="flex items-center gap-3">
                    ...
                    <div
                        class="w-8 h-8 bg-gradient-to-br from-green-500 to-green-700 rounded-lg flex items-center justify-center text-white shadow-sm">
                    ...
                    <div>
                        ...
                        <p class="text-[10px] text-green-600 font-semibold tracking-wider"
                            x-text="currentSessionId ? 'History Mode' : 'New Chat'"></p>
                    </div>
                </div>
            ...
            <template x-if="!hasStarted">
                <div class="flex-1 flex flex-col items-center justify-center px-4 overflow-y-auto">
                    <div
                        class="w-16 h-16 bg-gradient-to-br from-green-400 to-green-700 rounded-2xl flex items-center justify-center text-white shadow-lg mb-5">
                    ...
                    <p class="text-sm text-slate-500 dark:text-slate-400 mb-8 text-center max-w-md">
                        Saya adalah AI yang terhubung dengan data IoT kebun Anda. Tanyakan soal irigasi, hama, atau kondisi
                        cuaca.
                    </p>
            ...
            <template x-if="hasStarted">
                <div id="chatScrollArea" class="flex-1 overflow-y-auto space-y-6 p-6 scroll-smooth">
                    ...
                    {{-- Loading state saat load history --}}
                        ...
                        <template x-for="(msg, index) in messages" :key="index">
                            <div class="flex items-start gap-3 msg-animate"
                                ...
                                <template x-if="msg.role === 'bot'">
                                    <div
                                        class="w-8 h-8 rounded-full bg-gradient-to-br from-green-400 to-green-600 flex items-center justify-center text-white flex-shrink-0 shadow-sm mt-1">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path d="M13 10V3L4 14h7v7l9-11h-7z" stroke-width="2" stroke-linecap="round"
                                                stroke-linejoin="round"></path>
                                        </svg>
                                ...
                                <div :class="msg.role === 'user' ? 'user-bubble' : 'bot-bubble'"
                                    class="message-bubble px-5 py-4">
                                    <div class="bot-content-area" x-init="injectContent($el, msg)"></div>
                                    <span class="text-[10px] mt-2 block opacity-50 font-medium"
                                        :class="msg.role === 'user' ? 'text-white/70 text-right' : 'text-slate-500'"
                                        x-text="msg.time"></span>
                                </div>

                                <template x-if="msg.role === 'user'">
                                    <div
                                        class="w-8 h-8 rounded-full bg-gradient-to-br from-slate-300 to-slate-400 dark:from-slate-600 dark:to-slate-700 flex items-center justify-center text-slate-800 dark:text-white flex-shrink-0 font-bold text-xs shadow-sm mt-1">
                                        SA
                                    </div>
                                </template>
                            </div>
                    ...
                    <div x-show="isTyping" class="flex items-start gap-3">
                        <div
                            class="w-8 h-8 rounded-full bg-gradient-to-br from-green-400 to-green-600 flex items-center justify-center text-white flex-shrink-0 shadow-sm">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path d="M13 10V3L4 14h7v7l9-11h-7z" stroke-width="2" stroke-linecap="round"
                                    stroke-linejoin="round"></path>
                            </svg>
                        ...
                        <div class="bot-bubble px-5 py-4 flex items-center gap-1.5">
                            ...
                            <div class="w-2 h-2 bg-green-500 rounded-full dot-loading" style="animation-delay: 0.2s">
                            </div>
                            <div class="w-2 h-2 bg-green-500 rounded-full dot-loading" style="animation-delay: 0.4s">
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="p-4 bg-white dark:bg-slate-900 border-t border-slate-100 dark:border-slate-800">
                <div
                    class="relative flex items-center bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl shadow-sm chat-input max-w-4xl mx-auto dark:focus-within:bg-slate-800">
                    <input type="text" x-model="newMessage" @keydown.enter="sendChat()"
                        placeholder="Tanya tentang irigasi, hama, atau kondisi lahan..."
                        class="w-full bg-transparent text-slate-900 dark:text-slate-100 rounded-2xl py-4 pl-5 pr-14 focus:outline-none text-sm font-medium focus:ring-0 focus:bg-transparent dark:focus:bg-transparent">
                    <button @click="sendChat()" :disabled="!newMessage.trim() || isTyping"
                        ...
                        <svg class="w-5 h-5 transform -rotate-45 mb-0.5 ml-0.5" fill="none" stroke="currentColor"
                            viewBox="0 0 24 24">
    ...
    <script>
        document.addEventListener('alpine:init', () => {
            Alpine.data('chatbotComponent', () => ({
                ...
                // Fetch daftar sesi history dari server
                async fetchHistory() {
                ...
                // Load percakapan lama berdasarkan session_id
                async loadHistory(sessionId) {
                    ...
                    try {
                        ...
                        this.messages = [{
                            role: 'bot',
                            text: '⚠️ Gagal memuat percakapan dari server.',
                            time: ''
                        }];
                    } finally {
                ...
                // Dipanggil saat user mengganti dropdown lahan
                onLahanChange() {
                    this.startNewChat();
                    this.fetchHistory();
                },

                // Render konten markdown ke dalam elemen
                            ...
                            const type = () => {
                                if (i < fullText.length) {
                                    ...
                                    el.innerHTML = (typeof marked !== 'undefined') ?
                                        marked.parse(fullText.substring(0, i)) :
                ...
                // CSRF token langsung dari Laravel — tidak perlu meta tag
                csrfToken: '{{ csrf_token() }}',

                async fetchResponse(userText) {
                    try {
                        const params = new URLSearchParams();
                        params.append('question', userText);
                        params.append('lahan_id', this.selectedLahanId);
                        // Kirim session_id aktif agar percakapan tersambung (null = sesi baru)
                        ...
                        if (resJson.status === 'success') {
                            // Simpan session_id yang dikembalikan server (penting untuk sesi baru)
                            this.currentSessionId = resJson.data.session_id;
                            ...
                            // Refresh daftar history di sidebar setelah pesan baru tersimpan
                            this.fetchHistory();
                        } else {
                            throw new Error(resJson.message || 'Gagal memproses request');
                        }
                        ...
                        this.messages.push({
                            role: 'bot',
                            text: '⚠️ Gagal terhubung ke Server AI. Silakan coba lagi.',
                            time: 'Error'
                        });
                        this.scrollToBottom();
                    }
