...
@section('content')
    <div class="space-y-8 pb-10">
        ...
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            <div>
                <p class="text-xs font-bold uppercase tracking-[0.2em] text-slate-400 dark:text-slate-500 mb-1">SmarTani Admin</p>
        ...
        <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
            <div class="bg-white dark:bg-slate-900 rounded-2xl p-6 border border-slate-100 dark:border-slate-800 shadow-sm hover:shadow-md transition-shadow">
                <div class="flex items-start justify-between mb-4">
                    ...
                    <span class="text-xs font-bold text-blue-600 dark:text-blue-400 bg-blue-50 dark:bg-blue-900/20 px-2 py-0.5 rounded-full">Petani</span>
                </div>
            ...
            <div class="bg-white dark:bg-slate-900 rounded-2xl p-6 border border-slate-100 dark:border-slate-800 shadow-sm hover:shadow-md transition-shadow">
                <div class="flex items-start justify-between mb-4">
                    ...
                    <span class="text-xs font-bold text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-900/20 px-2 py-0.5 rounded-full">Lahan</span>
                </div>
            ...
            <div class="bg-white dark:bg-slate-900 rounded-2xl p-6 border border-slate-100 dark:border-slate-800 shadow-sm hover:shadow-md transition-shadow">
                <div class="flex items-start justify-between mb-4">
                    ...
                    <span class="text-xs font-bold text-violet-600 dark:text-violet-400 bg-violet-50 dark:bg-violet-900/20 px-2 py-0.5 rounded-full">IoT</span>
                </div>
            ...
            <div class="rounded-2xl p-6 border shadow-sm hover:shadow-md transition-shadow
                {{ $pendingDeviceCount > 0 ? 'bg-amber-50 dark:bg-amber-900/10 border-amber-200 dark:border-amber-800' : 'bg-white dark:bg-slate-900 border-slate-100 dark:border-slate-800' }}">
                <div class="flex items-start justify-between mb-4">
                    <div class="p-2.5 {{ $pendingDeviceCount > 0 ? 'bg-amber-100 dark:bg-amber-900/30' : 'bg-slate-100 dark:bg-slate-800' }} rounded-xl">
                        <svg class="w-5 h-5 {{ $pendingDeviceCount > 0 ? 'text-amber-600 dark:text-amber-400' : 'text-slate-400' }}" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
                        </svg>
                    </div>
                    @if ($pendingDeviceCount > 0)
                        <span class="text-xs font-bold text-amber-700 dark:text-amber-400 bg-amber-100 dark:bg-amber-900/30 px-2 py-0.5 rounded-full animate-pulse">Perlu aksi</span>
                    @endif
                </div>
                <p class="text-3xl font-black {{ $pendingDeviceCount > 0 ? 'text-amber-700 dark:text-amber-400' : 'text-slate-900 dark:text-white' }}">{{ $pendingDeviceCount }}</p>
                <p class="text-xs {{ $pendingDeviceCount > 0 ? 'text-amber-600/70 dark:text-amber-500' : 'text-slate-400' }} mt-1">Pending approval</p>
            </div>
        </div>

        {{-- SENSOR DATA PER LAHAN — SAMA SEPERTI DASHBOARD PETANI --}}
        @forelse($lahansWithSensors as $lahan)
            <div class="bg-white dark:bg-slate-900 rounded-[2.5rem] border border-slate-100 dark:border-slate-800 p-6 shadow-sm">
                <div class="flex items-center justify-between mb-6">
                    <div>
                        <h2 class="text-xl font-black text-slate-800 dark:text-white">{{ $lahan->nama_lahan }}</h2>
                        <p class="text-xs text-slate-500">Petani: {{ $lahan->petani->user->name ?? 'N/A' }} &middot; {{ $lahan->komoditas->nama_komoditas ?? 'N/A' }}</p>
                    </div>
                    <a href="{{ route('lahan.show', $lahan) }}" class="text-xs font-bold text-slate-400 hover:text-slate-600 transition-colors">Detail &rarr;</a>
                </div>

                <div class="grid grid-cols-1 lg:grid-cols-3 gap-4">
                    {{-- Card pH Tanah --}}
                    <div class="bg-gradient-to-br from-indigo-600 to-blue-700 rounded-[2rem] p-6 text-white shadow-lg">
                        <p class="text-[10px] font-bold opacity-80 uppercase tracking-widest mb-2">pH Tanah (Real-time)</p>
                        <div class="flex items-baseline gap-3">
                            <h2 class="text-4xl font-black tracking-tighter">{{ $lahan->soil_ph }}</h2>
                            <span class="bg-white/20 px-2 py-0.5 rounded-full text-[9px] font-bold backdrop-blur-md uppercase">
                                @if (is_numeric($lahan->soil_ph))
                                    {{ $lahan->soil_ph >= 5.5 && $lahan->soil_ph <= 7.5 ? 'Optimal' : 'Perlu Penyesuaian' }}
                                @else
                                    N/A
                                @endif
                            </span>
                        </div>
                        <p class="mt-4 text-[10px] opacity-70">Update: {{ $lahan->last_update }}</p>
                    </div>

                    {{-- Card Kelembapan Tanah --}}
                    <div class="bg-gradient-to-br from-emerald-600 to-teal-700 rounded-[2rem] p-6 text-white shadow-lg">
                        <p class="text-[10px] font-bold opacity-80 uppercase tracking-widest mb-2">Kelembapan Tanah</p>
                        <div class="flex items-baseline gap-3">
                            <h2 class="text-4xl font-black tracking-tighter">{{ $lahan->soil_moist }}</h2>
                            <span class="bg-black/10 px-2 py-0.5 rounded-full text-[9px] font-bold uppercase tracking-wider">%</span>
                        </div>
                        <p class="mt-4 text-[10px] opacity-70">Sensor ESP32 Aktif</p>
                    </div>

                    {{-- Info Lahan --}}
                    <div class="bg-slate-50 dark:bg-slate-800/50 rounded-[2rem] p-6 border border-slate-100 dark:border-slate-800">
                        <p class="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-3">Informasi Lahan</p>
                        <div class="space-y-2 text-sm">
                            <div class="flex justify-between">
                                <span class="text-slate-500">Komoditas</span>
                                <span class="font-bold text-slate-800 dark:text-white">{{ $lahan->komoditas->nama_komoditas ?? '-' }}</span>
                            </div>
                            <div class="flex justify-between">
                                <span class="text-slate-500">Luas</span>
                                <span class="font-bold text-slate-800 dark:text-white">{{ $lahan->luas_label }}</span>
                            </div>
                            <div class="flex justify-between">
                                <span class="text-slate-500">Device</span>
                                <span class="font-bold text-slate-800 dark:text-white">{{ $lahan->devices_count }} terpasang</span>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        @empty
            <div class="bg-white dark:bg-slate-900 rounded-[2.5rem] border border-slate-100 dark:border-slate-800 p-12 text-center shadow-sm">
                <p class="text-slate-400 text-sm">Belum ada data lahan.</p>
            </div>
        @endforelse
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
            @php
                $widgets = [
                    ['Suhu Udara', ($weatherData['temp'] ?? '--') . '°C', '🌡️'],
                    ['Kelembapan', ($weatherData['humidity'] ?? '--') . '%', '☁️'],
                    ['Kondisi', $weatherData['condition'] ?? 'N/A', '🌤️'],
                    ['Wilayah', $weatherData['area'] ?? 'Bandung', '📍'],
                ];
            @endphp
            @foreach ($widgets as $item)
                <div class="bg-white dark:bg-slate-900 p-6 rounded-[2rem] border border-slate-100 dark:border-slate-800 shadow-sm hover:shadow-md transition-shadow">
                    <div class="flex justify-between items-start mb-3">
                        <p class="text-[10px] font-bold text-slate-400 uppercase tracking-widest">{{ $item[0] }}</p>
            ...
            <div class="bg-white dark:bg-slate-900 rounded-2xl p-6 border border-slate-100 dark:border-slate-800 shadow-sm">
                ...
                <div class="flex gap-2 mb-4">
                    @php
                        ...
                        $offlinePct = $deviceCount ? round((max(0, $deviceCount - $onlineDeviceCount - $pendingDeviceCount) / $deviceCount) * 100) : 0;
                        $pendingPct = $deviceCount ? round(($pendingDeviceCount / $deviceCount) * 100) : 0;
                    @endphp
                    @if ($onlinePct > 0)
                        <div class="h-2 rounded-full bg-emerald-500 transition-all" style="width: {{ $onlinePct }}%"></div>
                    @endif
                    @if ($offlinePct > 0)
                        <div class="h-2 rounded-full bg-slate-300 dark:bg-slate-600 transition-all" style="width: {{ $offlinePct }}%"></div>
                    @endif
                    @if ($pendingPct > 0)
                        <div class="h-2 rounded-full bg-amber-400 transition-all" style="width: {{ $pendingPct }}%"></div>
                    @endif
                </div>
                <div class="flex flex-wrap gap-6 text-sm">
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        <span class="font-black text-slate-900 dark:text-white">{{ max(0, $deviceCount - $onlineDeviceCount - $pendingDeviceCount) }}</span>
                    </div>
                    <div class="flex items-center gap-2">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div class="bg-white dark:bg-slate-900 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm overflow-hidden">
                <div class="px-6 py-5 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between">
                    ...
                    <a href="{{ route('admin.lahan') }}"
                        class="text-xs font-bold text-slate-400 hover:text-slate-700 dark:hover:text-slate-200 transition-colors">Lihat semua →</a>
                </div>
                <div class="divide-y divide-slate-50 dark:divide-slate-800">
                    @forelse($recentLahans as $lahan)
                        <div class="flex items-center gap-4 px-6 py-4 hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                            <div class="w-9 h-9 rounded-xl bg-gradient-to-br from-emerald-400 to-emerald-600 flex items-center justify-center text-white font-black text-xs flex-shrink-0">
                                {{ strtoupper(substr($lahan->nama_lahan, 0, 2)) }}
                            </div>
                            <div class="flex-1 min-w-0">
                                <p class="font-bold text-slate-800 dark:text-white text-sm truncate">{{ $lahan->nama_lahan }}</p>
                                <p class="text-xs text-slate-400 truncate">{{ $lahan->petani->user->name ?? 'N/A' }}</p>
                            </div>
                            <div class="text-right flex-shrink-0">
                                <p class="font-black text-sm {{ $lahan->online_count > 0 ? 'text-emerald-600 dark:text-emerald-400' : 'text-slate-400' }}">
                                    {{ $lahan->online_count }}<span class="font-normal text-slate-300 dark:text-slate-600">/{{ $lahan->devices_count }}</span>
                                </p>
            ...
            <div class="bg-white dark:bg-slate-900 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm overflow-hidden">
                ...
                <div class="divide-y divide-slate-50 dark:divide-slate-800">
                    @forelse($users->take(6) as $user)
                        <div class="flex items-center gap-4 px-6 py-4 hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                            <div class="w-9 h-9 rounded-xl bg-gradient-to-br from-blue-400 to-blue-600 flex items-center justify-center text-white font-black text-xs flex-shrink-0">
                            ...
                            <div class="flex items-center gap-2 flex-shrink-0">
                                <span class="font-black text-sm text-slate-900 dark:text-white">{{ $user->lahan_count }}</span>
            ...
            <div class="bg-slate-900 dark:bg-slate-950 rounded-2xl border border-slate-800 shadow-sm p-6 text-white">
                ...
                <div class="space-y-4">
                    ...
                    <div class="flex items-center justify-between py-3">
                        ...
                        <span class="font-black text-xl {{ $pendingDeviceCount > 0 ? 'text-amber-400' : 'text-slate-500' }}">{{ $pendingDeviceCount }}</span>
                    </div>
                </div>
                @if ($pendingDeviceCount > 0)
