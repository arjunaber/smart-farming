        ...
        <form class="mt-8 space-y-6" method="POST" action="{{ route('login') }}">
            ...
            <div class="space-y-4">
                <div>
                    ...
                    <input id="email" name="email" type="email" value="{{ old('email') }}" required autofocus autocomplete="email" class="w-full px-4 py-3 border border-slate-200 rounded-xl focus:ring-2 focus:ring-green-500 focus:border-green-500 @error('email') border-red-500 @enderror bg-slate-50 dark:bg-slate-800">
                </div>
                <div>
                    <label for="password" class="block text-sm font-bold text-slate-500 uppercase tracking-wide mb-2">Password</label>
                    <input id="password" name="password" type="password" required autocomplete="current-password" class="w-full px-4 py-3 border border-slate-200 rounded-xl focus:ring-2 focus:ring-green-500 focus:border-green-500 @error('password') border-red-500 @enderror bg-slate-50 dark:bg-slate-800">
                </div>
                <div class="flex items-center">
                    <input id="remember" name="remember" type="checkbox" class="h-4 w-4 text-green-600 focus:ring-green-500 border-slate-300 rounded">
                    <label for="remember" class="ml-2 block text-sm text-slate-600 dark:text-slate-400">Ingat saya</label>
                </div>
            </div>
